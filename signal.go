...
func signalWatcher(ctx context.Context, logger log.Logger) (<-chan signalEvent, <-chan struct{}) {
	...
	// Buffer size ≥ number of distinct signals per os/signal docs: when
	// the handler goroutine is busy, the runtime can still deliver one of
	// each without dropping. We notify on 4 signals.
