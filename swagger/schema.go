...
// seen tracks visited types to prevent infinite recursion on
// self-referential structs (e.g. type Node struct { Next *Node }).
func schemaFromType(t reflect.Type, source string) *Schema {
	return schemaFromTypeR(t, source, make(map[reflect.Type]bool))
}

func schemaFromTypeR(t reflect.Type, source string, seen map[reflect.Type]bool) *Schema {
	...
	if t.Kind() != reflect.Struct {
		return typeSchemaR(t, seen)
	}
	if seen[t] {
		return &Schema{Type: "object"}
	}
	seen[t] = true
	defer delete(seen, t) // backtrack so sibling fields of the same type get full schemas

	s := &Schema{Type: "object", Properties: make(map[string]*Schema)}
	collectFieldsR(t, source, s, seen)
	return s
}

func collectFieldsR(t reflect.Type, source string, out *Schema, seen map[reflect.Type]bool) {
	for i := range t.NumField() {
		...
		if f.Anonymous {
			...
			if ft.Kind() == reflect.Struct {
				collectFieldsR(ft, source, out, seen)
			}
			continue
		}
...
func typeSchema(t reflect.Type) *Schema {
	return typeSchemaR(t, make(map[reflect.Type]bool))
}

func typeSchemaR(t reflect.Type, seen map[reflect.Type]bool) *Schema {
	// Unwrap all pointer layers (handles **T, *interface{}, etc.) before
	// type-switching, so every pointer depth resolves to the concrete kind.
	...
	case reflect.Slice:
		items := typeSchemaR(t.Elem(), seen)
		return &Schema{Type: "array", Items: items}
	case reflect.Map:
		valSchema := typeSchemaR(t.Elem(), seen)
		return &Schema{Type: "object", AdditionalProps: valSchema}
	case reflect.Interface:
		// any / interface{} — OpenAPI "object" is the closest equivalent
		return &Schema{Type: "object"}
	case reflect.Struct:
		return schemaFromTypeR(t, "json", seen)
	default:
		return &Schema{Type: "string"}
	}
}
