...
>
> **round-3 复审修正**（1H）：范围外分流的**定型时机**钉死——被标行
> 集合须按**原值**一次决定（固化集合再更新、或整列单条 CASE 就地
> 分流），不可在 blanket UPDATE 后重跑 =原值 谓词：CONVERT_TZ 的
> 越界判定作用于**输入值**的 FROM→UTC 中间值，已成功转换的结果按
> FROM 重读可能越出区间两端而误命中（上界：3001-01-18 20:00Z 东转
> +08 得 3001-01-19 04:00，按 UTC 重读越 cap 原样返回；下界：西转
> 对称），误标行吃静默二次重基——round-2 测试自身的两段式写法正是
> 此病（也违反配方「count≠0 不得 blanket」纪律）。测试改单条 CASE
> 按原值分流 + 扫描 pin 双臂（IS NULL=0、=原值=1），补上界穿越行
...
>
> **round-3~17 复审修正**（存量矩阵与操作纪律，实现不变）：旧基准实为**两个**
...
> 1960 史料过探针不被转；按命中臂分流：IS NULL 行=坏日期两路径
> 同 NULL、救不了归人工，仅 =原值 行改**带符号 interval** 区间
> 算术：东偏减西偏加、照抄正号反向错 10h，step-3 目标 TIMESTAMP
> 自身 1970–2038 存不进亦归人工）+ 单事务前核验全表
...
> （墙钟、不转 UTC，MySQL 手册明文；chok 当时固定 Loc=time.Local，
> 后由 #17 改为 UTC 双钉——见上方 #17 条），单进程
