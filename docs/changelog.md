...
> 路径无竞态且各租户拿到自己的列型与结果。
>
> **round-12 复审修正**（1M + 复审二过跟进 1H/1M，均红→绿）：
...
> fail-closed。改为事务读**只供本次调用、绝不发布**（也不经
> `db.AfterCommit` 延迟发布——那只省首个提交后聚合的一次合并读，不值
> 得引入 staging）；共享缓存的**读**路径事务保留：条目恒为已提交状态
> （唯一写入者是非事务 leader），事务自建的 relation 在契约内不可能
> 命中既有条目（PG 新表是新 OID；SQLite/MySQL 键 0 已有条目意味着表
> 早已存在、与事务内 CREATE 矛盾），剩余分歧即「存量 relation 的运行
> 期 DDL」= 文档化的重启契约——而绕过读会让天然在事务里的 SET LOCAL
> schema-per-tenant 流程（OID 分键的主要受众）每次聚合都重读元数据。
...
> 楔死 flight 证明事务不停靠**且不发布**、非事务等待者服从自身 ctx；
> 事务内建表→聚合→回滚不泄漏、后建 TEXT 表 fail-closed（修前静默
> 字典序）；取消 leader 后 8 健康等待者恰好 1 次读（修前 8 次）。
>
