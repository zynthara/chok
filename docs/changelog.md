...
> 路径无竞态且各租户拿到自己的列型与结果。
>
> **round-12 复审修正**（1M，红→绿）：round-11 声称的「同一 relation
> 并发首访至多重复一次元数据读」不成立——每个冷调用者都是 miss 后先
> 各自查完、`LoadOrStore` 才裁决缓存赢家：它去重的是**缓存**不是
> **加载**，K 个并发冷请求就发 K 条相同 catalog 读（复审以屏障在真 PG
> 稳定捕获 8/8）。改为按缓存键（PG=relation OID，SQLite/MySQL=0）
> singleflight 合并：同一波非事务调用者共享一次读，等待服从各自 ctx；
> 失败从不缓存，leader 被自己调用方取消时健康等待者各自在自己连接上
> 重试一次、不受牵连，真实 catalog 失败则整波共享一个错误。**显式事务
> 调用者不进 flight**、在自己连接上直读并照常 `LoadOrStore` 发布：其一
> 防死锁——停靠等待的事务持着连接，而 SQLite 写池钉死单连接
> （db/open.go），会构成 leader 等池、池等事务、事务等 leader 的环；
> 非事务等待者不持任何连接，等待图无环。其二保可见性——事务可能看到
> 别的连接看不到的 catalog（自身未提交 DDL、SET LOCAL search_path），
> 只有它自己的连接答得准；leader 恒为非事务连接，flight 共享、缓存的
> 恒是已提交 catalog。回归：8 并发冷首访恰好 1 次 catalog 读（修前
> 正是 8 次，无需屏障即稳定；按方言选 catalog 标记断言）；楔死 flight
> 证明事务调用者不停靠、非事务等待者服从自身 ctx。
>
