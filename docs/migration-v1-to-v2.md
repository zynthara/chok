...
可空列被写 NULL——软删行复活）；每条 UPDATE 前**逐目标列**扫描、
**两臂分开计数**（IS NULL 臂须 0：坏日期、DATE_SUB 同 NULL 救不了
→人工；=原值 臂相异固定偏移下允许 >0、即区间臂行集——范围外值
原样返回不报错，纯 blanket UPDATE 仅在此臂也为 0 时可用，否则走
固化集合/单条 CASE；
仅 =原值 行改**带符号** interval 区间算术——interval 恒为该条
CONVERT_TZ 的 FROM−目标 差：默认 UTC 目标即旧偏移自身（东偏正号
减、西偏负号加，照抄正号会反向），配了偏移目标必须按差算（UTC
存量转 +08:00 写 '-8:00' 即加 8h）；分流按**原值**一次定型（先
固化被标行集合、或整列单条 CASE 就地分流——blanket 更新后重评
=原值 谓词会把转换结果越界的已转换行误标二次重基）；step-3 恒用
旧进程−旧 session
差（与目标无关）、其目标 TIMESTAMP 自身 1970–2038 存不进
