...
- **时间聚合按瞬间比较，两条方言注记**。① SQLite 把时间戳按写入方时区
  ...
  配方相同）；**TIMESTAMP 列不迁**——内部存 UTC 瞬间，两半同偏移下
  ...
  **所配基准**（默认 UTC）的墙钟写入，否则同一瞬间两种墙钟、读取侧
  ...
  =原值 行走区间算术
  `DATE_SUB(col, INTERVAL '<±H:MM>' HOUR_MINUTE)`——**interval 带
  旧偏移自己的符号**：+08:00 写 '8:00' 是减、-05:00 写 '-5:00'
  负 interval 即加（照抄东偏正号会反向错 10h；step-3 回退用带符号
  的进程−session 差，且其目标是 **TIMESTAMP 列**、自身范围仅
