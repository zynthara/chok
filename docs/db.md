...
p, err := posts.Get(ctx, store.Where(                    // 条件定位，多行匹配取第一行
    where.WithFilter("status", "review")))
...
items, total, pageInfo, err := posts.ListFromQuery(ctx, r.URL.Query())

// 游标分页（深分页 / 无限滚动；O(1) 不随页深退化）
// 单列游标要求该列唯一 —— 用公开 id（RID，全局唯一）：
cp, err := posts.ListWithCursor(ctx, "id", where.CursorAfter, cursor, 20)
// cp.NextCursor 为空即最后一页；cp.Items 保证非 nil
```

> ⚠️ **游标列必须唯一**。按非唯一列（如 `created_at`）做单列游标时，同一时间戳
> 的多行会在页边界被静默跳过。要按 `created_at` 排序，改用复合游标
> `where.WithCursorBy("created_at", where.CursorAfter, fieldCursor, idCursor, size)`
> ——它追加内部 `id` 作 tie-breaker，把等值边界打散。
...
```go
// 投影单列；"id" 解析为公开 RID，内部数字键要用 PluckIDs
titles, err := store.Pluck[string](ctx, posts, "title",
    where.WithFilter("status", "published"))
statuses, err := store.PluckDistinct[string](ctx, posts, "status")   // 去重
...
err := posts.Update(ctx, store.RID(rid), store.Fields(p, "status").NoLock())
```
...
```go
store.Reader[T]       // Get / List / ListFromQuery / ListByIDs / Exists / Count
store.Writer[T]       // Create / Update / Delete / BatchCreate / Upsert
store.BatchWriter[T]  // BatchCreate / BatchUpdate / BatchUpsert（独立于 Writer，加批量能力不破坏既有 mock）
store.ReadWriter[T]   // Reader + Writer
```
