...
singleflight 合并成一次元数据读——显式事务调用者除外：它持连接
停靠会在有界连接池上与读取方互等，且事务可能看到自己未提交的
DDL，故它在自己连接上直读并照常发布），`SET LOCAL search_path` 的
...
>     "uk_email":           "email", // PG / MySQL：索引名
>     "email,delete_token": "email", // SQLite：列清单（复合索引按序逗号拼接）
> })
> ```
