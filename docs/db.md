...
- **没有 `ListWithCursor`**：keyset 游标的 tie-breaker 绑 RID 列，append
  ...
  **保证不漏**的消费属于 outbox 电池的领域
  （[§7.3](#73-transactional-outbox-电池outbox-模块)），不是 List 轮询
  能给的。
...
- **时间聚合按瞬间比较，两条方言注记**。① SQLite 把时间戳按写入方时区
  ...
  下发保证）。⚠️ **基准是持久化数据契约，不是进程偏好**：对**非空
  库**改 `mysql.time_zone`（默认 UTC→偏移、偏移 A→B、偏移改回 UTC
  同理）等同一次存量重基事件——新基准句柄会把旧基准写入的 DATETIME
  墙钟整体误读一个偏移差（排序/范围过滤/游标/聚合全部失真），须在
  启用新配置**之前**停写（该库全部写入方）+ 备份，对每个 DATETIME
  列（`deleted_at` 一并）执行
  `CONVERT_TZ(col, '<旧基准>', '<新基准>')`（相异数字偏移间精确
  无损；预扫描、范围外行、不可重复执行等执行纪律与下方存量迁移
  配方相同——但范围外行的 interval 回退按**旧基准 − 新基准**带符号
  写：UTC→+08:00 是 '-8:00' 即加 8h、+08:00→-05:00 是 '13:00'，
  照抄配方示例里「旧偏移自身」只对 UTC 目标成立；分流按**原值**
  一次定型——固化被标行集合或整列单条 CASE，blanket 更新后重评
  =原值 谓词会把「转换结果按旧基准重读越界」的已转换行误标、
  二次重基）；**TIMESTAMP 列不迁**——内部存 UTC 瞬间，两半同偏移下
  写读对称、任何基准下瞬间恒正确，盲转反而把正确瞬间搬歪；**DATE
  列不可重基**（历日无时刻）——存量历日的语义冻结为旧基准历日、新
  写入按新基准历日，须知即可；跳过条件是**旧基准 == 新基准**，不是
  「都是 UTC」。chok 写入方
  ...
  **所配基准**（默认 UTC）的墙钟写入，否则同一瞬间两种墙钟、读取侧
  ...
  求值列=旧 **session**（通常即服务器）时区），两者都等于**目标
  基准**（默认 UTC；配了 `mysql.time_zone` 即所配偏移——UTC 存量切
  偏移目标同样要重基）才可整体跳过。驱动写入的 DATETIME（created_at/updated_at/业务字段）按旧
  ...
  `deleted_at` 会被写成 NULL、软删行复活）；②**每条 UPDATE 前逐目标列
  扫描数据、两臂分开计数**（`at` 干净不代表 `deleted_at` 干净；两臂
  处置不同不可合并计数：**IS NULL 臂须为 0**——IS NULL 行=无效/零
  日期（合法数字偏移下 CONVERT_TZ 也 NULL、DATE_SUB 对同值同样
  NULL——fallback 救不了，可空列会换门复活），>0 即中止人工分类；
  **=原值 臂允许 >0、不是停止信号**——两个相异固定偏移下真转换必
  移动值，原样返回的正是区间臂要救的范围外行集（超出 CONVERT_TZ
  支持范围的值**原样返回不报错**，1960 年史料能过探针却不被转换），
  但纯 blanket UPDATE 仅在此臂也为 0 时可跑、任何正数都改走固化
  集合/单条 CASE 分流；「=原值即漏转」的判据只对相异固定偏移成立，
  命名时区的零偏移瞬间会正确地原样返回——被标行本就走应用侧路径
  无碍；=原值 行走区间算术
  `DATE_SUB(col, INTERVAL '<±H:MM>' HOUR_MINUTE)`——**interval 恒为
  该条 CONVERT_TZ 的 FROM − TO 带符号差**：本配方默认 UTC 目标下
  即旧偏移自身的符号（+08:00 写 '8:00' 是减、-05:00 写 '-5:00'
  负 interval 即加，照抄东偏正号会反向错 10h）；目标为所配偏移时
  必须按差算——UTC 存量转 +08:00 写 '-8:00'（加 8h，「旧偏移自身」
  是减 0）、+08:00 转 -05:00 写 '13:00'（不是 8h）；分流本身须按
  **原值**一次定型——先固化被标行集合再更新、或整列单条 CASE
  （`WHEN 转换=原值 THEN 区间臂 ELSE CONVERT_TZ END`）就地分流，
  **不可在 blanket UPDATE 之后重跑 =原值 谓词**：已转换值按 FROM
  重读时可能越出支持区间两端而误命中（3001-01-18 20:00 经 UTC→+08
  转成 3001-01-19 04:00，再按 UTC 读越上界原样返回），被误标行吃
  静默的第二次重基；step-3 回退
  恒用带符号的进程−session 差（它救的那条转换两端就是两个旧时区，
  与目标无关），且其目标是 **TIMESTAMP 列**、自身范围仅
  1970–2038——近 epoch 行 fallback 结果存不进、严格模式响亮拒绝，
  同归人工），命名时区的走应用侧处置）；③**单
...
> 提交写"的场景不要挂在这上面**——那需要事务性 outbox：
> [§7.3](#73-transactional-outbox-电池outbox-模块)。

### 7.3 Transactional outbox 电池（outbox 模块）

bus 承载不了的「必须看到每一次已提交写」由 outbox 电池承载：**同事务
入队 + scheduler 扫描投递**，传递保证 **at-least-once**、按
(created_at, 内部 PK) 全序、崩溃不丢。
...
chok.Use(db.Module(...), scheduler.Module(), outbox.Module(
    outbox.WithRelay("projector", func(ctx context.Context, rec outbox.Record) error {
        return project(ctx, rec.Topic, rec.Payload) // 返回 error = 本轮停摆，下个 tick 重试
    }),
))

// 业务侧：与业务写同一事务，要么都提交要么都回滚
ob := outbox.From(k)
err := h.RunInTx(ctx, func(txCtx context.Context) error {
...
- **事务门禁**：`Enqueue` 必须收到 outbox 自己句柄的 `RunInTx` txCtx，
  否则 `outbox.ErrOutsideTx`——脱离事务的入队丢掉原子性，正是这块电池
  要消灭的半成品，直接报错不降级。别的句柄的事务同样被拒（那不与业务
  写同事务）。
...
- **失败 = head-of-line**：Handler 报错即停本轮、水位线停在失败行之前，
  ...
  其他 relay）。
- **多 relay 各自独立**：`WithRelay` 可注册多个（`OnTopics(...)` 过滤
