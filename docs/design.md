...
- **聚合正门**（arch-backlog #7）：`store.Sum[N]` / `Avg` / `Min[N]` /
  ...
  依赖；同库实例同偏移由配置下发保证；基准=**持久化数据契约**——对
  非空库改基准是一次停写重基事件：DATETIME 逐列 CONVERT_TZ 旧→新，
  TIMESTAMP 瞬间基准无关不迁、盲转即腐蚀，DATE 历日冻结不可重基，
  跳过条件=旧基准==新基准）：chok 写入方结构性免疫 DST
  ...
  **非 chok 写入方**须同按所配基准（默认 UTC）墙钟写（DATETIME 不带
  ...
  合并、取消波次回流二次合并（显式事务调用者不进合并、在自己连接上
  直读，**提交后**才经 after-commit 缓冲发布、回滚丢弃——事务私有
  可见性不得在提交前进共享缓存，防有界池上的等待环；缓存条目恒为
  已提交状态，事务的缓存读与回暖都保留）。表名解析与数据查询同规则：限定
  ...
  enum。scope 先于 catalog 读：未认证请求在纯内存 fail-closed 即 401、
  不碰数据库（catalog 读会跑真实查询）。类型收敛是显式契约：
