...
| `db_migration_expected_version` / `db_migration_applied_version` | Gauge | `instance` |
| `db_migrations_applied` / `db_migrations_pending` / `db_migrations_dirty` | Gauge | `instance` |
...
Prometheus scrape 路径执行 SQL。这样滚动发布时可以区分「旧二进制仍在
