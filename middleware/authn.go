...
// maxAuthorizationLen caps the Authorization header length the middleware
// will look at. Anything larger is treated as malformed and rejected
// without invoking the parser. A real HS256 JWT carrying typical claims
// is ~1 KB; 8 KB leaves comfortable headroom for nested-claim tokens
// while keeping a single oversized header from feeding 1 MB+ payloads
// into jwt.Parse (which parses linearly and runs HMAC over the whole
// signing input, an easy single-core DoS vector).
...
func extractBearer(header string) string {
	...
	// Trim leading/trailing whitespace from the token so inputs like
	// "Bearer   abc" (multiple spaces after the scheme) still parse.
	// jwt.Parse rejects tokens with leading whitespace.
