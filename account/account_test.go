...
func TestRegister_EmptyName_UsesmaskedEmail(t *testing.T) {
	...
	user, err := m.store.Get(context.Background(), where.WithFilter("email", "alice@test.com"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestForgotAndResetPassword(t *testing.T) {
	...
	if sender.lastTo != "alice@test.com" {
		t.Fatalf("sender.to = %q, want alice@test.com", sender.lastTo)
	}
	if sender.lastCode == "" {
...
func TestResetPassword_InvalidToken(t *testing.T) {
	_, r := setupModule(t, WithSender(&mockSender{}))
...
func TestSetup_WithSender(t *testing.T) {
	...
	if sender.lastTo != "bob@test.com" {
		t.Fatalf("sender.to = %q, want bob@test.com", sender.lastTo)
	}
}
...
func TestActiveCheck_DisabledUser_Blocked(t *testing.T) {
	...
	user.Active = false
	if err := m.store.UpdateOne(context.Background(), user, "active"); err != nil {
		t.Fatal(err)
	}
