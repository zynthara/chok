...
func Migrate(gdb *gorm.DB, specs ...TableSpec) error {
...
func createSoftUniqueIndex(gdb *gorm.DB, model any, idx SoftIndex) error {
	...
	case "mysql":
		...
		return gdb.Exec(fmt.Sprintf("CREATE UNIQUE INDEX %s ON %s (%s)",
			idx.Name, tableName, colList)).Error
	default:
