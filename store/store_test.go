...
func setupUserStore(t *testing.T) (*Store[User], *gorm.DB) {
	t.Helper()
	gdb := setupDB(t)
	if err := db.Migrate(gdb, db.Table(&User{}, db.SoftUnique("uk_email", "email"))); err != nil {
		t.Fatal(err)
	}
...
func setupItemStore(t *testing.T) *Store[Item] {
	t.Helper()
	gdb := setupDB(t)
	if err := db.Migrate(gdb, db.Table(&Item{})); err != nil {
		t.Fatal(err)
	}
...
func TestBatchCreate_SingleFailure_Rollback(t *testing.T) {
	...
	_, err = s.Get(context.Background(), where.WithFilter("email", "new1@b.com"))
...
func TestUpdateOne_OptimisticLock(t *testing.T) {
	...
	u.Name = "alice2"
	if err := s.UpdateOne(context.Background(), u, "name"); err != nil {
		t.Fatal(err)
	}
	if u.Version != 2 {
...
func TestUpdateOne_ZeroColumns_ErrMissingColumns(t *testing.T) {
	s, _ := setupUserStore(t)
	u := createUser(t, s, "alice", "alice@example.com")

	err := s.UpdateOne(context.Background(), u)
...
func TestUpdateOne_NotFound(t *testing.T) {
	...
	err := s.UpdateOne(context.Background(), u, "name")
	if !errors.Is(err, ErrNotFound) {
...
func TestUpdateOne_StaleVersion(t *testing.T) {
	...
	u2, _ := s.Get(context.Background(), where.WithFilter("id", u.RID))
	...
	err := s.UpdateOne(context.Background(), u2, "name")
...
func TestGet_NotFound(t *testing.T) {
	s, _ := setupUserStore(t)
	_, err := s.Get(context.Background(), where.WithFilter("id", "usr_nonexistent"))
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound, got %v", err)
	}
}
...
func TestGet_Found(t *testing.T) {
	...
	found, err := s.Get(context.Background(), where.WithFilter("id", created.RID))
	if err != nil {
		t.Fatal(err)
	}
	if found.Name != "alice" {
...
func TestGetOne(t *testing.T) {
	...
	found, err := s.GetOne(context.Background(), created.RID)
	if err != nil {
		t.Fatal(err)
	}
	if found.Name != "alice" {
...
func TestGetOne_NotFound(t *testing.T) {
	s, _ := setupUserStore(t)
	_, err := s.GetOne(context.Background(), "usr_nonexistent")
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound, got %v", err)
	}
}
...
func TestAutoAlias_IdMapsToRid(t *testing.T) {
	...
	found2, err := s.Get(context.Background(), where.WithFilter("id", created.RID))
	if err != nil {
		t.Fatal(err)
	}
...
func TestList_EmptyResult(t *testing.T) {
	s, _ := setupUserStore(t)
	items, total, err := s.List(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if items == nil {
	...
	if total != -1 {
		t.Fatalf("expected total -1 (no count), got %d", total)
	}
}

func TestList_DefaultTotal_NegativeOne(t *testing.T) {
	s, _ := setupUserStore(t)
	createUser(t, s, "alice", "a@b.com")

	_, total, _ := s.List(context.Background())
	if total != -1 {
		t.Fatalf("expected -1 without WithCount, got %d", total)
	}
}
...
func TestList_WithCount(t *testing.T) {
	...
	items, total, err := s.List(context.Background(), where.WithCount())
	if err != nil {
		t.Fatal(err)
	}
	if len(items) != 2 {
...
func TestDeleteMany_Idempotent(t *testing.T) {
	...
	err := s.DeleteMany(context.Background(), where.WithFilter("id", "usr_nonexistent"))
	if err != nil {
...
func TestDeleteMany_SoftDelete_SetsDeleteToken(t *testing.T) {
	...
	if err := s.DeleteMany(context.Background(), where.WithFilter("id", u.RID)); err != nil {
		t.Fatal(err)
	}

	// Should not be found via normal query (soft-deleted).
	_, err := s.Get(context.Background(), where.WithFilter("id", u.RID))
	if !errors.Is(err, ErrNotFound) {
...
func TestDeleteMany_PhysicalDelete(t *testing.T) {
	...
	if err := s.DeleteMany(context.Background(), where.WithFilter("id", item.RID)); err != nil {
		t.Fatal(err)
	}

	_, err := s.Get(context.Background(), where.WithFilter("id", item.RID))
	if !errors.Is(err, ErrNotFound) {
...
func TestWithQueryFields_UnknownField_Rejected(t *testing.T) {
	s, _ := setupUserStore(t)
	_, err := s.Get(context.Background(), where.WithFilter("unknown_field", "val"))
	if err == nil {
...
func TestWithQueryFields_MapsCorrectly(t *testing.T) {
	...
	found, err := s.Get(context.Background(), where.WithFilter("id", u.RID))
	if err != nil {
		t.Fatal(err)
	}
	if found.Name != "alice" {
...
func TestWithQueryFields_NotConfigured_Rejects(t *testing.T) {
	gdb := setupDB(t)
	db.Migrate(gdb, db.Table(&Item{}))
	s := New[Item](gdb, log.Empty()) // no WithQueryFields

	_, err := s.Get(context.Background(), where.WithFilter("code", "ABC"))
	if err == nil {
...
func TestWithColumnAlias_BeforeFields_WorksCorrectly(t *testing.T) {
	...
	got, err := s.Get(context.Background(), where.WithFilter("id", item.RID))
	if err != nil {
...
func TestWithOrder_FieldValidation(t *testing.T) {
	...
	// Valid field.
	_, _, err := s.List(context.Background(), where.WithOrder("name"))
	if err != nil {
		t.Fatalf("valid order field should work: %v", err)
	}

	// Unknown field.
	_, _, err = s.List(context.Background(), where.WithOrder("nonexistent"))
	if err == nil {
...
func TestTransaction_SingleStore(t *testing.T) {
	...
	err := s.Transaction(context.Background(), func(tx *Store[User]) error {
	...
	found, err := s.Get(context.Background(), where.WithFilter("email", "tx@b.com"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransaction_Rollback(t *testing.T) {
	...
	_, err = s.Get(context.Background(), where.WithFilter("email", "rb@b.com"))
	if !errors.Is(err, ErrNotFound) {
...
func TestGet_OnlyOrderOption_ErrMissingConditions(t *testing.T) {
	...
	_, err := s.Get(context.Background(), where.WithOrder("code"))
	if !errors.Is(err, ErrMissingConditions) {
...
func TestDeleteMany_OnlyOrderOption_ErrMissingConditions(t *testing.T) {
	...
	err := s.DeleteMany(context.Background(), where.WithOrder("code"))
	if !errors.Is(err, ErrMissingConditions) {
...
func TestList_WithCount_IgnoresPagination(t *testing.T) {
	...
	if total != 5 {
		t.Fatalf("expected total 5 (all items), got %d", total)
	}
}
...
func TestStore_IndirectSoftDelete(t *testing.T) {
	gdb := setupDB(t)
	if err := db.Migrate(gdb, db.Table(&IndirectUser{}, db.SoftUnique("uk_inu_email", "email"))); err != nil {
		t.Fatal(err)
	}
...
func TestSoftUnique_ActiveRecordUniqueness(t *testing.T) {
	...
	u, _ := s.Get(context.Background(), where.WithFilter("email", "alice@example.com"))
	if err := s.DeleteMany(context.Background(), where.WithFilter("id", u.RID)); err != nil {
		t.Fatal(err)
	}
...
func TestWithFilterOp_InvalidOp_Error(t *testing.T) {
	...
	_, err := s.Get(context.Background(), where.WithFilterOp("name", where.Op("OR 1=1 --"), "x"))
	if err == nil {
...
func TestWithFilterOp_ValidOps(t *testing.T) {
	...
	for _, op := range []where.Op{where.Eq, where.Ne, where.Gt, where.Gte, where.Lt, where.Lte} {
		_, _, err := s.List(context.Background(), where.WithFilterOp("name", op, "alice"))
		if err != nil {
...
func TestWithUpdateFields_UnknownField_Rejected(t *testing.T) {
	...
	u.Name = "alice2"
	err := s.UpdateOne(context.Background(), u, "created_at")
	if err == nil {
...
func TestDefaultAutoDiscoverUpdateFields(t *testing.T) {
	...
	if err := s.UpdateOne(context.Background(), item, "code"); err != nil {
		t.Fatalf("auto-discovered update field should work: %v", err)
	}

	// Base model fields should be rejected.
	err := s.UpdateOne(context.Background(), item, "version")
	if err == nil {
...
func TestDeleteOne_Idempotent(t *testing.T) {
	...
	err := s.DeleteOne(context.Background(), "usr_nonexistent", 0)
	if err != nil {
...
func TestDeleteOne_NotFound_WithVersion(t *testing.T) {
	s, _ := setupUserStore(t)
	err := s.DeleteOne(context.Background(), "usr_nonexistent", 1)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound, got %v", err)
	}
}
...
func TestDeleteOne_StaleVersion(t *testing.T) {
	...
	err := s.DeleteOne(context.Background(), u.RID, 1)
	if !errors.Is(err, ErrStaleVersion) {
		t.Fatalf("expected ErrStaleVersion, got %v", err)
	}
...
func TestDeleteOne_CorrectVersion(t *testing.T) {
	...
	err := s.DeleteOne(context.Background(), u.RID, u.Version)
	if err != nil {
...
func TestDeleteOne_SoftDelete_SetsDeleteToken(t *testing.T) {
	...
	if err := s.DeleteOne(context.Background(), u.RID, 0); err != nil {
		t.Fatal(err)
	}
...
func TestDeleteOne_PhysicalDelete(t *testing.T) {
	...
	if err := s.DeleteOne(context.Background(), item.RID, 0); err != nil {
		t.Fatal(err)
	}
...
func TestAutoDiscoverUpdateFields_ExcludesBaseModel(t *testing.T) {
	...
	err := s.UpdateOne(context.Background(), item, "id")
	if err == nil {
...
func TestResolveUpdateColumn_UnknownField_ErrUnknownUpdateField(t *testing.T) {
	...
	err := s.UpdateOne(context.Background(), u, "nonexistent")
...
func TestWhereErrUnknownField(t *testing.T) {
	s, _ := setupUserStore(t)
	_, _, err := s.List(context.Background(), where.WithFilter("bogus", "val"))
...
func TestDefaultAutoDiscoverQueryFields(t *testing.T) {
	...
	got, err := s.Get(context.Background(), where.WithFilter("code", "ABC"))
	if err != nil {
	...
	_, err = s.Get(context.Background(), where.WithFilter("bogus", "x"))
	if err == nil {
...
func TestScope_List_Filtered(t *testing.T) {
	...
	if len(items) != 1 || items[0].Name != "alice" {
		t.Fatalf("expected only alice, got %d items", len(items))
	}
}
...
func TestScope_UpdateOne_Blocked(t *testing.T) {
	...
	bob.Name = "hacked"
	err := s.UpdateOne(context.Background(), bob, "name")
	if !errors.Is(err, ErrNotFound) {
...
func TestScope_DeleteOne_Blocked(t *testing.T) {
	...
	err := s.DeleteOne(context.Background(), item.RID, item.Version)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound (scope blocks delete), got %v", err)
	}

	// Item should still exist.
	_, err = sAll.GetOne(context.Background(), item.RID)
	if err != nil {
...
func TestScope_Error_FailClosed(t *testing.T) {
	...
	_, err := s.GetOne(context.Background(), "rid_x")
	...
	_, err = s.Get(context.Background(), where.WithFilter("code", "x"))
	if !errors.Is(err, scopeErr) {
	...
	err = s.UpdateOne(context.Background(), item, "code")
	if !errors.Is(err, scopeErr) {
		t.Fatalf("UpdateOne: expected scope error, got %v", err)
	}
	err = s.DeleteOne(context.Background(), item.RID, 0)
	if !errors.Is(err, scopeErr) {
...
func TestScope_NoScope_NoEffect(t *testing.T) {
	...
	got, err := s.GetOne(context.Background(), u.RID)
	if err != nil {
		t.Fatal(err)
	}
	if got.Name != "test" {
