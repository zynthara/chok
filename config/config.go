...
// AuditOptions configures the audit log Component (parts/audit). Audit
// log is the compliance-grade record of "who did what to which
// resource"; distinct from access log (HTTP traffic) and metrics
// (aggregate counters). Disabled by default — enable when retention
// and queryability are explicit product requirements.
//
// Reload semantics (parts/audit Reload):
//   - reload-safe : RetentionDays / PurgeInterval / PurgeBatchSize
//   - restart only: AsyncBufferSize (channel capacity, not resizable
//     mid-flight) and DropOnFull (changes producer-side blocking
//     semantics — switching live would surprise hot-path callers).
type AuditOptions struct {
	...
	// is governed by DropOnFull. Restart-only (see Reload semantics).
	...
	// holding row locks long enough to stall INSERT. Reload-safe.
	PurgeBatchSize int `mapstructure:"purge_batch_size" default:"1000"`

	// EnableAdminAPI registers GET /audit/logs (gated by
...
// Validate enforces the SPEC §8.5 invariants. Non-discriminator —
// AuditOptions does NOT implement SelfValidating; the framework's
// recursive walker handles its single level of nested fields fine.
