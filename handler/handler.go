...
func scanTags(t reflect.Type, binders []Binder) map[string]bool {
	...
	for i := range t.NumField() {
		...
		if f.Anonymous && f.Type.Kind() == reflect.Struct {
			for tag, found := range scanTags(f.Type, binders) {
				if found {
					present[tag] = true
				}
			}
...
func HandleRequest[T any, R any](h HandlerFunc[T, R], opts ...HandleOption) gin.HandlerFunc {
	cfg := &handleConfig{successCode: http.StatusOK, binders: defaultBinders}
	for _, o := range opts {
		o(cfg)
	}
...
func HandleAction[T any](h ActionFunc[T], opts ...HandleOption) gin.HandlerFunc {
	cfg := &handleConfig{successCode: http.StatusNoContent, binders: defaultBinders}
	for _, o := range opts {
		o(cfg)
	}
	active := activeBinders(reflect.TypeOf((*T)(nil)).Elem(), cfg.binders)
...
func resolveError(ctx context.Context, err error) *apierr.Error {
	...
	// Registered mappers (e.g. store.MapError).
	if mapped := apierr.Resolve(err); mapped != nil {
		return mapped
	}
